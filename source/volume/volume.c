...
void volumeInt_printPlaneXY(VolumeInt* volume, Int z) {
  ASSERT_VOLUME(volume);

  ASSERT_VOLUME_INDEX_Z(volume, z);
  printf("xy-plane at z = %lld:\n", z);

  for (Int y = 0; y < volumeInt_getHeight(volume); y += 1) {
    ASSERT_VOLUME_INDEX_Y(volume, y);
    printf("  all x at y = %lld: ", y);
    printf("(");
    
    for (Int x = 0; x < volumeInt_getWidth(volume); x += 1) {
      ASSERT_VOLUME_INDEX_X(volume, x);

      printf("%lld", volumeInt_getAt(volume, x, y, z));
      if (x < volumeInt_getWidth(volume)-1) {
	printf(", ");
      } else {
	printf(")\n");
      }
...
void volumeInt_print(VolumeInt* volume) {
  ASSERT_VOLUME(volume);

  for (Int z = 0; z < volumeInt_getDepth(volume); z += 1) {
    ASSERT_VOLUME_INDEX_Z(volume, z);

    volumeInt_printPlaneXY(volume, z);
  }
}
